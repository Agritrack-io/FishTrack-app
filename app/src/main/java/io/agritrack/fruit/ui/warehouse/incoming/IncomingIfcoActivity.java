package io.agritrack.fruit.ui.warehouse.incoming;
...
import io.agritrack.fruit.ui.warehouse.inventory.IfcoInventoryActivity;
...
public class IncomingIfcoActivity extends LocationAwareActivity {

    private final TransactionApi updService = APIServiceGenerator.createAPI(TransactionApi.class);
    private MobileDB db;

    private RecyclerView rvIncomingIfco;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_incoming_ifco);

        // activate GPS location update feature.
        super.findLocation();

        // set Header Info
        TextView tvHeader = findViewById(R.id.tvHeaderIncomingIfco);
        ...
        progressDialog = new ProgressDialog(IncomingIfcoActivity.this);
        ...
        ivSupport.setOnClickListener(view -> {
            supportDialog = new SupportDialog(IncomingIfcoActivity.this);
        ...
        ivNext.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mLastLocation != null) {
                    recIncoming.longitude = mLastLocation.getLongitude();
                    recIncoming.latitude = mLastLocation.getLatitude();
                } else {
                    CToast(IncomingIfcoActivity.this, "Error: Unable to get Location from GPS", Toast.LENGTH_LONG);
                }
    ...
    private void assignCtrlVars() {
        rvIncomingIfco = findViewById(R.id.rvIncomingIfco);
    ...
    private boolean updateState() {
        if (adapterIfco != null) {
            recIncoming.ifcoItems = adapterIfco.getValues();
        }
        String v = validate();
        ...
        FruitGlobalState.recIncoming.state = WarehouseTxState.Incoming;
        FruitGlobalState.recIncoming.consumableType = ConsumableType.valueOf(Constants.ftIfco);
        ...
        try {
            ...
            IncomingTxWithItems tx = FruitGlobalState.commitIncomingIfco(db);

            // sync WH Incoming Tx
            Call<ConsumableTxDTO> syncIncomingTxAsyncCall = updService.syncIncomingTx(ConsumableTxDTO.convertIncoming(tx), "Bearer " + token);
            syncIncomingTxAsyncCall.enqueue(new IncomingIfcoActivity.SyncIncomingTxCallBack());
    ...
    private String validate() {
        ...
        if (!IsDemo) {
            if (FruitGlobalState.recIncoming.ifcoItems == null || FruitGlobalState.recIncoming.ifcoItems.isEmpty()) {
                sb.append(String.format("\n%s is missing", "'Ifco items'"));
            }
        }
    ...
    public class SyncIncomingTxCallBack implements Callback<ConsumableTxDTO> {
        @Override
        public void onResponse(Call<ConsumableTxDTO> call, Response<ConsumableTxDTO> response) {
            ConsumableTxDTO rs = response.body();

            if (rs != null || IsDemo) {
                runOnUiThread(() -> CToast(getApplicationContext(), render("Tx successfully updated!!!"), Toast.LENGTH_LONG));
            } else {
                // could not update Fishing TX on backend!!!
                runOnUiThread(() -> CToast(getApplicationContext(), render("Incoming transaction update failure!!!"), Toast.LENGTH_LONG));
            }
        }

        @Override
        public void onFailure(Call<ConsumableTxDTO> call, Throwable error) {
